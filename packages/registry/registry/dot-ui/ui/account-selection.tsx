...
import { DialogClose, DialogHeader, DialogTitle, DialogDescription, DialogFooter } from "@/components/ui/dialog";
...
export default function AccountSelection() {
    ...
    useEffect(() => {
        if (connectedAccount && accounts.map((account) => account.address).includes(connectedAccount.address)) {
            return;
        }
                ...
                {accounts.map((account) => (
                    <DialogClose asChild key={account.address}>
                        <Button onClick={() => setConnectedAccount(account)}>
                            ...
                            <span>{formatBalance(balances[account.address]?.free || 0, network)}</span>
                        </Button>
                    </DialogClose>
                ))}
            </DialogHeader>
