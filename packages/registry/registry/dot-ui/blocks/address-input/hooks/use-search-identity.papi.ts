...
/**
 * Searches for blockchain identities on the "paseo_people" network whose display names include the specified string.
 *
 * Uses React Query to asynchronously fetch and filter identity entries from the chain, returning results that match the provided display name (case-insensitive, minimum 3 characters). The query is enabled only when the API is connected and the display name is valid.
 *
 * @param displayName - The display name substring to search for; must be at least 3 characters to trigger a search
 * @returns A React Query result containing an array of matching identity search results, each with an address and formatted identity fields
 */
