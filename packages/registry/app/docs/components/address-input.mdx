...
import { AddressInputWithProvider } from "@/registry/polkadot-ui/blocks/address-input/components/address-input.papi";
...
<AddressInput
  format="ss58"
  identityChain="paseoPeople"  // Specify which chain to use for identity lookup
  withIdentityLookup={true}
/>
```
</TabsContent>
...
| `identityChain` | `ChainIdWithIdentity` | `"paseoPeople"` | Chain to use for identity lookups |
...
- **`paseoPeople`** (default): Paseo People chain for testnet identities
