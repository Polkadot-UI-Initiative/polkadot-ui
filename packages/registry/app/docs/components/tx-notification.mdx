...
- **URL Format**: `https://{chain}.subscan.io/extrinsic/{txHash}`
...
```tsx
// Always provide links for transaction verification
<TxNotification
  explorerUrls={{
...
```tsx
interface CustomTxMonitor {
  onSignatureRequest: () => void;
  onTransactionSubmitted: (hash: string) => void;
  onBlockInclusion: (blockHash: string, blockNumber: number) => void;
  onFinalization: (txHash: string) => void;
...
<TxNotification
  onTxStatusChange={(status) => {
    ...
    customMonitor.handleStatusChange(status);
  }}
/>
```
...
<TxNotification
  onTxStatusChange={(status) => {
    ...
    analytics.track('transaction_status_change', {
      state: status.state,
