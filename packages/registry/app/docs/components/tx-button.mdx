...
title: Tx Button
description:
  A configurable transaction button with fee preview, progress states, and
  notifications.
---
...
If you want to use txNotifications too, you need to add the <Toaster />
component from sonner to your app. It is bundled with the component.
...
function RemarkExample() { const tx = useTx((tx) => tx.system.remark) return (
<TxButtonBase tx={tx} args={["hello"]} networkId="paseo"> Write a message on
chain </TxButtonBase> ) }
