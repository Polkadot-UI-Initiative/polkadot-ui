...
export class ListCommand {
     ...
     */
    async validateRegistryConnection() {
        ...
        try {
            ...
            spinner.succeed(`Connected to registry (${registryInfo.componentCount} components available)`);
            if (this.options.dev) {
     ...
     */
    showConnectionGuidance(registryUrl) {
        logger.newline();
        if (registryUrl.includes("localhost")) {
            ...
            logger.info("You can test it by opening http://localhost:3000/registry.json in your browser");
        }
        else {
 ...
 */
export function createListCommand(options) {
